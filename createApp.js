...
const createApp = async (config) => {
  ...
  try {
    ...
    console.log(
      `${colors.red("Error!")} We were unable to download the project template.`,
    );
    return;
  }
  ...
  // Create local .env
  await copyFile(
    join(config.projectDirectory, "example.env"),
    join(config.projectDirectory, ".env"),
  );
  ...
  // Success!
  console.log(
    `${colors.green("Success!")} Created ${config.appName} at ${config.projectDirectory}`,
  );
  ...
  console.log(
    `  ${colors.cyan(`${config.packageManager} ${config.packageManager === "yarn" ? "" : "run "}dev`)}`,
  );
};
